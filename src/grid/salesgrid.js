	...
	render() {
		return (
			<form onSubmit={this.handleSubmit}>
				...
				<a href="#" onClick={this.yesterday}>
					[Yesterday]
				</a>
				<a href="#" onClick={this.previousDay}>
					[ &lt; ]
				</a>
				<a href="#" onClick={this.nextDay}>
					[ &gt; ]
				</a>
				<a href="#" onClick={this.monthToDate}>
					[Month to Date]
				</a>
				...
				<img
					height="20"
					width="20"
					className={this.props.loading ? "loading" : "loaded"}
					src={loadingSpinner}
					alt="loading"
				/>
		...
		return (
			<thead>
				<tr>
					<th colSpan="2" />
					{addons.map((addon, i) => (
						<th className="addon" colSpan="5" key={i}>
							{addon.name}
						</th>
					))}
				</tr>
...
class MarketplaceDashboardRow extends Component {
	renderTransactions(transactions) {
		const { sales, renewals, upgrades, downgrades, refunds } = transactions,
			total = {
				count:
					sales.count +
					renewals.count +
		...
		return [sales, renewals, upgrades, downgrades, refunds, total].map(
			this.transaction
		);
	}
	...
	render() {
		...
		return (
			<tr>
				...
				<td>{label}</td>
				{addons.map((addon) =>
					this.renderTransactions(addon[type.toLowerCase()][label])
				)}
			</tr>
		);
	}
...
class MarketplaceDashboardFooter extends Component {
	render() {
		...
		let totals = addons.map((addon) => ({
			total: {
				Cloud: Object.keys(addon.cloud).reduce(
					(total, tier) => {
						...
						total.downgrades.count += addon.cloud[tier].downgrades.count;
						total.downgrades.amount += addon.cloud[tier].downgrades.amount;
					...
					},
					{
						...
						upgrades: { count: 0, amount: 0 },
						downgrades: { count: 0, amount: 0 },
						refunds: { count: 0, amount: 0 },
						total: { count: 0, amount: 0 },
					}
				),
				Server: Object.keys(addon.server).reduce(
					(total, tier) => {
						...
						total.downgrades.count += addon.server[tier].downgrades.count;
						total.downgrades.amount += addon.server[tier].downgrades.amount;
		...
		totals.forEach((addon) => {
			addon.total.All = Object.keys(addon.total).reduce(
				(total, type) => {
					...
					total.downgrades.count += addon.total[type].downgrades.count;
					total.downgrades.amount += addon.total[type].downgrades.amount;
		...
		const grandTotal = {
			count: totals.reduce(
				(total, addon) => (total += addon.total.All.total.count),
				0
			),
			amount: totals.reduce(
				(total, addon) => (total += addon.total.All.total.amount),
				0
			),
		};
		...
		return (
			<tfoot>
				...
				/>
				<tr>
					<td colSpan={2} className="transactions">
						({grandTotal.count}){" "}
						{grandTotal.amount.toLocaleString("en-au", {
							style: "currency",
							currency: "AUD",
						})}
					</td>
...
export default class AddOnSalesGrid extends Component {
	...
	setRange(startDate, endDate) {
		...
		this.fetchData(startDate, endDate)
			...
			.then((data) =>
				this.setState({ startDate, endDate, data, loading: false })
			);
	}
		...
		return fetch(url, { headers })
			...
			.then((json) => {
				...
				return json._links.next
					? this.fetchData(startDate, endDate, offset + 50, transactions)
					: transactions;
			})
			.catch(console.log);
	}
	...
	transform(transactions) {
		return transactions.reduce((addons, transaction) => {
			...
			let { hosting, tier, saleType, vendorAmount } =
			...
			addons[name][hosting][tier][saleType] = addons[name][hosting][tier][
				saleType
			] || { count: 0, amount: 0 };
	...
	mergedTiers(type, transactions) {
		return (tiers, tier) => {
			if (transactions && transactions[type] && transactions[type][tier]) {
				...
				tiers[tier].renewals =
					transactions[type][tier].Renewal || tiers[tier].renewals;
				tiers[tier].upgrades =
					transactions[type][tier].Upgrade || tiers[tier].upgrades;
				tiers[tier].downgrades =
					transactions[type][tier].Downgrade || tiers[tier].downgrades;
				tiers[tier].refunds =
					transactions[type][tier].Refund || tiers[tier].refunds;
			}
	...
	merge(data) {
		return {
			addOns: this.state.addOns.map((addon) => {
				...
				addon.cloud = this.cloudTiers.reduce(
					this.mergedTiers("Cloud", transactions),
					addon.cloud
				);
				addon.server = this.serverTiers.reduce(
					this.mergedTiers("Server", transactions),
					addon.server
				);
